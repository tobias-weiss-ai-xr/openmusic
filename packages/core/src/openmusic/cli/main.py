...
@click.option("--title", default=None, help="YouTube video title (auto-generated if omitted)")
@click.option("--description", default="", help="YouTube video description")
@click.option(
    "--tags",
    default="dub techno,electronic music,AI generated",
...
@click.option(
    "--schedule",
    default=None,
    help="Scheduled publish time in ISO 8601 (e.g. 2026-06-27T12:00:00Z)",
)
@click.option(
    "--thumbnail",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option(
    "--playlist",
    default=None,
    help="YouTube playlist title to add the video to",
)
@click.option(
    "--client-secrets",
    default=None,
    help="Path to client_secrets.json for YouTube OAuth",
)
@click.option(
    "--no-effects",
    is_flag=True,
...
def release(
    ...
    A streamlined one-shot command for releasing a mix to YouTube.
    Auto-generates video metadata from the mix parameters if not provided.
    """
    ...
    # Auto-generate title from mix params if not specified
    if not title:
        title = f"{key} Deep Dub Techno | {length} @{bpm}BPM"

    # Auto-fill default description with GitHub link
    if not description:
        description = (
            f"AI-powered dub techno mix ({key}, {bpm}BPM, {length}).\n"
            f"Generated by OpenMusic — AI-powered dub techno generation\n"
            f"https://graphwiz.ai\n"
            f"https://github.com/tobias-weiss-ai-xr/openmusic"
        )
    ...
    click.echo("\n[2/3] Rendering MP4 video...")
    try:
        mp4_path = str(_Path(output).with_suffix(".mp4"))
        stem = _Path(output).stem
        parent = _Path(output).parent

        # Find cover image (PNG preferred, SVG fallback)
        cover_png = parent / f"{stem}_cover.png"
        cover_svg = parent / f"{stem}_cover.svg"
        ...
        if cover_png.exists():
            ...
            # Generate cover via CoverGenerator if openart not available
            try:
                from openmusic.export.cover_generator import CoverGenerator, MixCoverConfig

                gen = CoverGenerator(MixCoverConfig(
                    key=key, bpm=bpm, length=seconds, title=title,
                ))
                gen.save_svg(str(cover_svg))
                gen.save_png(str(cover_png))
                cover_input = str(cover_png)
            except ImportError:
                dub_visual = _Path(__file__).parent.parent.parent.parent.parent / "dub_visual.svg"
                if dub_visual.exists():
                    _subprocess.run(
                        ["rsvg-convert", "-w", "1920", "-h", "1080", str(dub_visual), "-o", str(cover_png)],
                        check=True, capture_output=True,
                    ...
                    raise click.ClickException(
                        "No cover art available. Install openart or provide a custom cover."
                    )
    ...
    click.echo("\n" + "=" * 50)
    click.echo("[OK] Release complete!")
    click.echo(f"  Title: {title}")
    click.echo(f"  URL: https://youtube.com/watch?v={video_id}" if video_id else "")
    return 0
