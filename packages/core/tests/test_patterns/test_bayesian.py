...
class TestBayesianPatternSelector:
    ...
    def library(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            yield PatternLibrary(path=db_path)

    def test_empty_candidates_returns_none(self, library):
        """Returns None when no candidates are provided."""
        selector = BayesianPatternSelector(library)
        result = selector.select([])
        assert result is None

    def test_selects_from_single_candidate(self, library):
    ...
    def test_preferences_higher_quality_candidates(self, library):
        ...
        library.add_many([high_quality, low_quality])
    ...
    def test_quality_feedback_updates_scores(self, library):
    ...
    def test_select_increments_play_count(self, library):
        """Selecting a pattern increments its play count in the library."""
        pattern = PatternEntry(
            path="test.wav", duration=30.0, bpm=125, key="Dm",
        )
        library.add(pattern)
